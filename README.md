...
它的目标不是替代 IDA，而是让 AI 在不打开 IDA 图形界面的情况下，读取 IDA 的函数、基本块、CFG 信息，并结合覆盖率数据快速回答：
...
- DynamoRIO `drcov`（仓库已内置 Windows 版，路径 `third_party/dynamorio`）
...
BeaconFlow 支持两种工作流，取决于目标架构和可用工具：

### 工作流 A：IDA + DynamoRIO（x86/x64 Windows 目标）

适用于 IDA 能打开的目标。先用 IDA headless 导出 metadata，再用 DynamoRIO 采集覆盖率。
...
`-d in_asm` 更像翻译块日志，hit count 不应当当作精确循环次数。需要精确执行次数时用 `-d exec,nochain` 或后续接更强 trace。
