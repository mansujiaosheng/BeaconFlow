...
- 单个基本块的反汇编上下文、前驱/后继、附近比较、调用、字符串、常量和 inspect 优先级原因
- 指定函数/地址范围的执行流分析
- 后续哪些函数值得重点分析或 fuzz
...
BeaconFlow 不自研 hook/符号执行/调试框架，只负责**推荐模板 → 填充参数 → 导入输出 → 总结证据**。
...
python -m beaconflow.cli import-gdb-log --log gdb_output.log
python -m beaconflow.cli import-angr-result --result angr_solution.json
...
## DynamoRIO 自定义 Instrumentation

```powershell
python -m beaconflow.cli dr-generate-client --type compare --output dr_compare.cpp
python -m beaconflow.cli dr-run-client --client dr_compare.dll --target target.exe --output-dir runs
```
...
python -m beaconflow.cli schema --validate report.json --name coverage_report
python -m beaconflow.cli schema --validate-all <case_dir>
```

## Case 工作区质量检查

```powershell
python -m beaconflow.cli case-check --root <case_dir>
```

检查内容：metadata 存在性、run 信息、report 存在性、ai_digest、evidence_id、confidence、target 文件、大文件、schema 匹配、next_actions。

***

## HTML 报告

所有分析命令的 `--format` 参数新增 `html` 和 `html-json` 选项，可直接输出带暗色主题样式的 HTML 报告：

```powershell
# 输出 HTML 格式报告（从 Markdown 转换）
python -m beaconflow.cli analyze --metadata metadata.json --coverage sample.drcov --format html --output report.html

# 输出 HTML 格式报告（从 JSON 转换）
python -m beaconflow.cli analyze --metadata metadata.json --coverage sample.drcov --format html-json --output report.html
```

也可以用独立的 `to-html` 命令将已有的 Markdown 或 JSON 报告转换为 HTML：
...
- 增加 apk-native-summary（APK native SO 分析）
...
- ✅ triage-pyc（Python .pyc 一键分析）
- ✅ HTML 报告输出（html/html-json 格式 + to-html 命令）
- ✅ Benchmark cases（标准化测试用例框架）
- ✅ 状态变量恢复（recover-state-transitions）
- ✅ IDA/Ghidra 标注脚本输出（export-annotations）
- ✅ 多 trace 合并（deflatten-merge）
- ✅ 模板库与建议引擎（templates.py + suggest-hook/angr/debug）
- ✅ 外部工具输出导入（import-frida-log/gdb-log/angr-result/jadx-summary）
- ✅ 一键 Triage 工作流（triage-native/qemu/wasm/pyc/apk）
- ✅ Fuzz Corpus 管理（fuzz_corpus.py）
- ✅ DynamoRIO 自定义 Instrumentation（dynamorio_custom.py）
- ✅ 稳定 JSON Schema（schemas.py）
- ✅ 报告置信度系统（report_confidence）
- ✅ 统一 ai_digest 字段
- ✅ APK 薄适配（AXML manifest 解析 + native libs 检测 + Android hook 推荐）
- ✅ Schema 批量验证（schema --validate-all）
- ✅ Case 工作区质量检查（case-check）
